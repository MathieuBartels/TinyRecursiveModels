...
class TrainState:
    ...
    total_steps: int
    accum_counter: int = 0
    pending_metric_keys: Optional[List[str]] = None
    pending_metric_values: Optional[Any] = None
...
def init_train_state(
    ...
    # Optimizer steps (effective batch = global_batch_size * accum_steps)
    total_steps = int(
        config.epochs
        * train_metadata.total_groups
        * train_metadata.mean_puzzle_examples
        / (config.global_batch_size * config.accum_steps)
    )
...
def save_train_state(config: PretrainConfig, train_state: TrainState):
    ...
    if config.upload_checkpoints_to_wandb and wandb.run is not None:
        # Upload the exact checkpoint file so it is available in W&B run files.
...
def _effective_batch_size(config: PretrainConfig, global_batch_size: int) -> int:
    return global_batch_size * config.accum_steps


def _accumulate_micro_batch_metrics(
    ...
    if train_state.pending_metric_keys is None:
        train_state.pending_metric_keys = metric_keys
        train_state.pending_metric_values = metric_values.copy()
    else:
        train_state.pending_metric_values += metric_values


def _finalize_pending_train_metrics(
    config: PretrainConfig,
    train_state: TrainState,
    global_batch_size: int,
    lr_this_step: float,
) -> Optional[dict]:
    if train_state.pending_metric_keys is None or train_state.pending_metric_values is None:
        return None

    reduced_metrics = {
        k: train_state.pending_metric_values[i]
        for i, k in enumerate(train_state.pending_metric_keys)
    }
    train_state.pending_metric_keys = None
    train_state.pending_metric_values = None

    effective_batch = _effective_batch_size(config, global_batch_size)
    count = max(reduced_metrics["count"], 1)
    reduced_metrics = {
        f"train/{k}": v / (effective_batch if k.endswith("loss") else count)
...
def _optimizer_step(
    ...
    assert lr_this_step is not None
    return _finalize_pending_train_metrics(
        config, train_state, global_batch_size, lr_this_step
    )
...
def flush_accumulated_grads(
    ...
    if train_state.accum_counter == 0:
        return None, False
    ...
    metrics = _optimizer_step(
        config, train_state, global_batch_size, rank, world_size
    )
    return metrics, True
...
def train_batch(
    ...
    effective_batch = _effective_batch_size(config, global_batch_size)
    ((1 / effective_batch) * loss).backward()

    if len(metrics):
        _accumulate_micro_batch_metrics(train_state, metrics, rank, world_size)

    train_state.accum_counter += 1
    if train_state.accum_counter < config.accum_steps:
        return None, False

    return (
        _optimizer_step(config, train_state, global_batch_size, rank, world_size),
        True,
    )
...
def launch(hydra_config: DictConfig):
    ...
    for _iter_id in range(total_iters):
        ...
        for set_name, batch, global_batch_size in train_loader:
            metrics, did_optimizer_step = train_batch(
                config,
                train_state,
                batch,
                global_batch_size,
            ...
            if config.ema and did_optimizer_step:
            ...
            if (
                ...
                and config.checkpoint_interval_steps > 0
                and train_state.step > 0
                and train_state.step % config.checkpoint_interval_steps == 0
            ):
                print(f"SAVE CHECKPOINT (step interval): {train_state.step}")
                save_train_state(config, train_state)

        metrics, did_optimizer_step = flush_accumulated_grads(
            config,
            train_state,
